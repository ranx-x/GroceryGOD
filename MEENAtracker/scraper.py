...
def run_scrapers():
    ...
    try:
        res_web = subprocess.run([sys.executable, web_script], cwd=dir_path, capture_output=True, text=True, timeout=3600)
    ...
    try:
        res_app = subprocess.run([sys.executable, app_script], cwd=dir_path, capture_output=True, text=True, timeout=1800)
