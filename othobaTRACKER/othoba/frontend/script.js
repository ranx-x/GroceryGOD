...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        document.title = "Othoba Price Monitor";
        showLoading(true, 'Loading Othoba products...');
    ...
    try {
        ...
        allProducts = data.map(p => {
            ...
            return {
                ...
                minPrice: old || price, maxPrice: price,
...
async function loadAllFromParquet() {
    ...
    let pBuf, hBuf;
    try {
        [pBuf, hBuf] = await Promise.all([
            fetch('products.parquet').then(r => {
                if (!r.ok) throw new Error(`products.parquet HTTP ${r.status}`);
                log(`products.parquet: ${r.status} ${(r.headers.get('content-length')||'?')} bytes`);
                return r.arrayBuffer();
            }),
            fetch('history.parquet').then(r => {
                if (!r.ok) throw new Error(`history.parquet HTTP ${r.status}`);
                log(`history.parquet: ${r.status} ${(r.headers.get('content-length')||'?')} bytes`);
                return r.arrayBuffer();
            })
        ]);
    } catch (fetchErr) {
        ...
        const jsonOk = await loadAllFromJson();
        if (jsonOk) { log('Fell back to HAR JSON data'); godDB = { db, conn }; return; }
        showLoading(true, `PARQUET LOAD FAILED: ${fetchErr.message}`, 100);
        throw fetchErr;
    }
    ...
    try {
        ...
        log(`REGISTER FAILED: ${regErr.message}`);
        showLoading(true, `PARQUET REGISTER FAILED: ${regErr.message}`, 100);
        throw regErr;
    }
    ...
    try {
        ...
        log(`SQL QUERY FAILED: ${queryErr.message}`);
        showLoading(true, `ERROR: ${queryErr.message}`, 100);
        throw queryErr;
    }
...
async function loadProductHistory(productId) {
    ...
    try {
        ...
        console.error(`[GOD] loadProductHistory failed for ${productId}:`, err);
        return [];
    }
}
...
async function computePriceChanges(days) {
    ...
    try {
        ...
        console.error(`[GOD] computePriceChanges query failed:`, err);
        return;
    }
    const oldPrices = {};
    for (const row of result.toArray()) {
...
function renderSidebar() {
    ...
    Object.keys(STORE_CONFIG).forEach(sid => {
        ...
        // Shop header = select all checkbox
...
async function loadStoreData(sid) {
    if (window[sid + 'Manifest']?.metadata) metadata.stores[sid] = window[sid + 'Manifest'].metadata;
}
...
function updateStoreStats() {
    ...
    sortedStores.forEach(([store, data]) => {
        ...
        <div class="legend-item" style="display:flex; flex-direction:column; margin-bottom:10px;">
            ...
            <div style="font-size:0.6rem; opacity:0.6; color:#888;">Range: ${data.date_range || 'N/A'}${data.categories ? ' | '+data.categories.length+' cats' : ''}${data.source==='har' ? ' 📡' : ''}</div>
        </div>`;
    });
