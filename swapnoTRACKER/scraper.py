...
def ensure_lang_en(url):
    if not url: return ""
    if 'lang=' in url: return url
    sep = '&' if '?' in url else '?'
...
async def self_scrape_items(page, container_selector, category, current_data, today_str, summary, is_pinned=False):
    ...
    for item in items:
        try:
            ...
            raw_url = f"https://www.shwapno.com{url_suffix}" if url_suffix.startswith('/') else url_suffix
            product_url = ensure_lang_en(raw_url)
...
async def capture_and_send_ss(page, caption=""):
    try:
        ...
        logger.warning(f"Failed to capture/send browser screenshot: {ex}")
...
async def scrape_category(sem, browser, category, current_data, summary, pinned_names=[]):
    async with sem:
        ...
        target_url = ensure_lang_en(category['url'])
        logger.info(f"Scraping: {category['name']} ({target_url})")
        try:
            ...
            if "Attention Required" in title or "Cloudflare" in title or "Just a moment" in title:
                logger.error(f"  [X] Blocked by Cloudflare on {category['name']}")
                await capture_and_send_ss(page, f"Shwapno Cloudflare Block: {category['name']}")
                return False
            ...
            await capture_and_send_ss(page, f"Shwapno Error ({category['name']}): {str(e)[:80]}")
            return False
        finally:
            await context.close()
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        if failed:
            logger.warning(f'  [!] {failed}/{len(results)} categories hit timeout or errored')
        await browser.close()
